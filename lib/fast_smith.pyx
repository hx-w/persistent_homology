...
def low(int i, np.ndarray[np.uint8_t, ndim=2] matrix):
    cdef np.ndarray[np.uint8_t, ndim=1] col
    cdef int j
...
@cython.boundscheck(False)
@cython.wraparound(False)
def isReduced(np.ndarray[np.uint8_t, ndim=2] matrix, int bound):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def bitwise_xor(np.ndarray[np.uint8_t, ndim=1] col_i,
                np.ndarray[np.uint8_t, ndim=1] col_j,
...
@cython.boundscheck(False)
@cython.wraparound(False)
def smith_normal_form(np.ndarray[np.uint8_t, ndim=2] matrix):
    cdef int i, j, n, m
    cdef np.ndarray[np.uint8_t, ndim=1] col_i, col_j, out
    cdef np.ndarray[np.uint8_t, ndim=2] reduced_matrix
